...
public struct SwizzleReplacer {
  ...
  func callAsFunction(_ type: AnyClass) {
    guard let originalMethod = class_getInstanceMethod(type, original),
      ...
      logger.warning("Failed to swizzle: #selector(\(original.description)) => #selector(\(swizzled.description))")
      return
    }
...
extension Selector {
  public static func => (original: Selector, swizzled: Selector) -> SwizzleReplacer {
