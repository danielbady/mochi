          ...
          HStack(spacing: 0) {
            Color.clear
              ...
              .gesture(
                TapGesture(count: 2)
                  .onEnded { _ in
                    viewStore.send(.didSkipBackwards)
                  }
              )
...
extension Loadable {
  fileprivate func videoContentState(for content: VideoPlayerFeature.State.VideoPlayerState.ContentType) -> VideoPlayerFeature.State.VideoPlayerState? {
    switch self {
    case .pending:
